...
typedef struct {
    ...
    /* Per-layer MoE routing */
    bf16_t **moe_gate;       /* [n_layers]: [n_routed_experts, hidden_size] */

    /* Per-layer routed expert FFN weights: [n_layers][n_experts] */
    ...
    /* Per-layer shared expert FFN weights */
 ...
 * Real checkpoints differ from the idealized layout (e.g. dsv2lite has q_proj,
 * not q_a/q_b; layer 0 is dense) — this surfaces that cleanly. */
...
static void free_model_weights(ModelWeights *w, int n_layers) {
    ...
    free(w->q_a_proj);    free(w->q_b_proj);  free(w->kv_a_proj);
...
void build_transformer(Transformer *t, const char *index_path, const char *shard_dir) {
    /* 1. load config (hardcoded for Phase 0) */
    t->config = (Config){
...
int main(int argc, char *argv[]) {
    ...
    Transformer t;
    build_transformer(&t, index_path, shard_dir);
    ...
    /* Smoke test: spot-check a few key tensors. */
